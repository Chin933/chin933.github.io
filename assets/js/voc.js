(function () {
  ...
  var observedMarkup = 14.9;
  var baselineFutureWeight = Math.pow(0.96, 10);
  ...
  function model(epsilon, maturation, discount) {
    ...
    var competitiveQuantity = Math.pow(monopolyMarkup, epsilon);
    var observedQuantity = Math.pow(observedMarkup / monopolyMarkup, -epsilon);
    var restrictionSlope = Math.log(competitiveQuantity / observedQuantity);
    var maxStaticProfit = monopolyMarkup - 1;
    var futureWeight = Math.pow(discount, maturation);
    var timingPenalty = 0.50 * (baselineFutureWeight - futureWeight);

    function commercialReturn(x) {
      var quantity = competitiveQuantity * Math.exp(-restrictionSlope * x);
      var price = monopolyMarkup * Math.pow(quantity, -1 / epsilon);
      return ((price - 1) * quantity) / maxStaticProfit - timingPenalty * x;
    }

    var points = [];
    var best = { x: 0, value: -Infinity };
    for (var i = 0; i <= 210; i += 1) {
      var x = plot.xmax * i / 210;
      var value = commercialReturn(x);
      points.push({ x: x, value: value });
      if (value > best.value) best = { x: x, value: value };
    }
    var historicalValue = commercialReturn(1);
    return {
      points: points,
      optimum: best.x,
      maxReturn: best.value,
      historicalReturn: historicalValue,
      lossShare: Math.max(0, (best.value - historicalValue) / best.value)
    };
  }
  ...
  function render() {
    ...
    var ymin = Math.min(0, result.historicalReturn - 0.05);
    var ymax = Math.max(1.08, result.maxReturn * 1.08);
    ...
    [0, 0.25, 0.5, 0.75, 1].forEach(function (tick) {
      ...
      add('text', { x: 91, y: yScale(tick) + 6, 'text-anchor': 'end', fill: '#59616a', 'font-family': 'Arial, sans-serif', 'font-size': 17 }, Math.round(tick * 100));
      add('text', { x: xScale(tick), y: 591, 'text-anchor': 'middle', fill: '#59616a', 'font-family': 'Arial, sans-serif', 'font-size': 17 }, Math.round(tick * 100) + '%');
    });
    ...
    add('text', { x: xScale(result.optimum), y: optimumY - 24, 'text-anchor': 'middle', fill: '#7e5c23', 'font-family': 'Arial, sans-serif', 'font-size': 16, 'font-weight': 700 }, 'COMMERCIAL OPTIMUM');
    ...
    add('text', { x: xScale(1) - 16, y: historicalY - 24, 'text-anchor': 'end', fill: '#84372d', 'font-family': 'Arial, sans-serif', 'font-size': 16, 'font-weight': 700 }, 'HISTORICAL SCALE');
    add('text', { x: (xScale(result.optimum) + xScale(plot.xmax)) / 2, y: 104, 'text-anchor': 'middle', fill: '#9d5a50', 'font-family': 'Arial, sans-serif', 'font-size': 15, 'font-weight': 700, 'letter-spacing': 1.5 }, 'SELF-DEFEATING RESTRICTION');
    add('text', { x: 600, y: 649, 'text-anchor': 'middle', fill: '#30353a', 'font-family': 'Arial, sans-serif', 'font-size': 19, 'font-weight': 700 }, 'SUPPLY-RESTRICTION INTENSITY');
    add('text', { x: 31, y: 320, 'text-anchor': 'middle', fill: '#30353a', 'font-family': 'Arial, sans-serif', 'font-size': 19, 'font-weight': 700, transform: 'rotate(-90 31 320)' }, 'DISCOUNTED GROSS RETURN (PEAK = 100)');
    ...
    document.getElementById('voc-profit-loss').textContent = Math.round(result.lossShare * 100) + '%';
